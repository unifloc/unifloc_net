...
Module Module1
    ...
    Sub Main()
        ...
        Dim pvt_str
        'test_pvt = u7_excel.u7_Excel_function_servise.PVT_decode_string()
        'Dim pvt_str, rhg
        ...
        Console.WriteLine(test_choke(0))
        Console.ReadKey(True)

        Dim test_q_choke
        test_q_choke = u7_excel.u7_Excel_functions_MF.MF_q_choke_sm3day(fw_perc:=20, d_choke_mm:=15, p_in_atma:=60, p_out_atma:=50, d_pipe_mm:=70, t_choke_C:=20, c_calibr_fr:=test_choke(0), str_PVT:=pvt_str)
        Console.WriteLine(test_q_choke(0))
        Console.ReadKey(True)
